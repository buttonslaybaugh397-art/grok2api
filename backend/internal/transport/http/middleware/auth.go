...
// AdminAuth validates admin Bearer JWT tokens.
func AdminAuth(service *adminauth.Service) gin.HandlerFunc {
	return func(c *gin.Context) {
		raw, ok := bearerToken(c.GetHeader("Authorization"))
		if !ok {
			response.Error(c, http.StatusUnauthorized, "adminUnauthorized", "admin login expired")
			return
		}
		value, err := service.AuthenticateAccess(c.Request.Context(), raw)
		if err != nil {
			if errors.Is(err, adminauth.ErrRuntimeUnavailable) {
				response.Error(c, http.StatusServiceUnavailable, "authRuntimeUnavailable", "admin auth service unavailable")
				return
			}
			response.Error(c, http.StatusUnauthorized, "adminUnauthorized", "admin login expired")
			return
...
// ClientAuth validates downstream API keys and releases concurrency leases after the request.
func ClientAuth(service *clientkeyapp.Service) gin.HandlerFunc {
	return func(c *gin.Context) {
		// Browser CORS preflight must pass without requiring a client key.
...
func clientErrorMessage(err error) string {
	if errors.Is(err, clientkeyapp.ErrRuntimeUnavailable) {
		return "gateway runtime is temporarily unavailable, please retry later"
	}
	return err.Error()
}
